...
describe('redis', function(){
  ...
  before(function(){ 
    ...
    mockery.registerMock('ioredis', {
      createClient(config){
        console.log(config)
        return  { get(key){ return prom(db__[key]) }
                , set(key, value){  db__[key] = value; console.log(db__)} 
                }
      }
    })
  })
  ...
  it('should create connection hooks', function(){  
    var ripple = db(redis(data(core())), { db: { redis: 'redis://user:pass@localhost:6379/genius' }})
    console.log(ripple.connections)
    expect(keys(ripple.connections)).to.be.eql(['redis'])
    expect(ripple.connections.redis.change).to.be.a('function')
  })

  // it('should create correct load and behaviour', function(){  
    // var ripple = db(redis(data(core())), { db: { redis: 'redis://user:pass@localhost:6379/genius' }})
    // ripple('foo', [1,2,3])
    // expect(_db).to.be.eql({'foo': [1,2,3]})
  ...
  it('should create correct insert behaviour', function(){  
    var ripple = db(redis(data(core())), { db: { redis: 'redis://user:pass@localhost:6379/genius' }})
      , record = { name: 'foo' }

    ripple('foo', [])
    push(record)(ripple('foo'))
    expect(db__).to.eql({'foo': { name: 'foo'}})
  })
